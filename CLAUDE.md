...
- **Tab timing inference** (`utils/tabParser.js`): bar lines make each measure one bar of the time signature, event starts proportional to column position (sixteenth-quantized); barline-free staves fall back to uniform eighths. Sequential same-string notes that quantize onto one grid slot (hammer-on pairs in dense bars) are nudged apart — a beat may never contain two notes on one string. Articulations (`7h9`, `9p7`, `5/7`) are captured as `event.technique` ('hammer'|'pull'|'slide'), shown as superscript glyphs in the tab, and played softer.
- **Beat grouping** (`score.js` `START_EPSILON = 0.125`): events are simultaneous only when starts effectively coincide; the epsilon must stay strictly below the 0.25 quantization grid or adjacent slots merge into impossible chords.
- **Instrument clearing**: in the Chords view (chord selected) and Scale view, the piano/fretboard show only that context — song dots and the song capo never leak across tabs (`chordFocus` gates in Piano/Fretboard).
...
- **Capo** (`score.meta.capo`): tab sources with a capo directive (regex `capo\D{0,12}?(\d{1,2})` anywhere in the text) store events capo-RELATIVE, exactly as written. `deriveBeats` applies `capo + transpose` as one shift, so sounding pitch/notation/piano/audio are true while tab numerals stay capo-relative. Fingering (`fingering.js`) works in relative frets — a capo'd open is finger 0 and chord shapes match the library at their relative position (an Em shape at capo 7 reads "Em" even though it sounds Bm). The fretboard draws a capo bar at `capo + transpose` with open notes ringed at the bar; the transport shows a CAPO chip.
...
- **Scale capo** (`scaleCapo` in store, stepper in the sidebar): optional; reframes the whole explorer above the chosen fret — positions below it vanish, CAGED/diagonal patterns recompute (`computeScaleFretPositions(notes, minFret)`), the capo bar is drawn, and notes at the capo render open-style. Playback sequences respect it.
