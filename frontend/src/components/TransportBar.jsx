...
function Stepper({ value, onDecrement, onIncrement, children, label }) {
  return (
    <div className="flex items-center gap-0.5" title={label}>
      ...
      >−</button>
      {children ?? (
        <span className="font-mono text-sm text-chrome-100 min-w-[2.5rem] text-center">{value}</span>
      )}
      <button
        onClick={onIncrement}
        className="w-6 h-6 rounded-md text-chrome-400 hover:text-chrome-100 hover:bg-ink-800 transition-colors text-sm leading-none"
...
export default function TransportBar() {
  ...
  const hasContent = beats.length > 0 && !scaleViewActive;
  const progress = hasContent ? currentBeat / Math.max(1, beats.length - 1) : 0;
  const matchedChord = hasContent ? beats[currentBeat]?.matchedChord : null;

  const handlePlayToggle = () => {
    if (!hasContent) return;
    if (!isPlaying) {
      unlockAudio();
      // Restart from the top if the song already finished
      if (currentBeat >= beats.length - 1) setCurrentBeat(0);
    }
  ...
  return (
    <footer className="flex items-center gap-5 px-5 h-16 border-t border-ink-700/60 bg-ink-900/95 backdrop-blur relative z-10 flex-shrink-0">
      ...
      >
        <PlayIcon playing={isPlaying && hasContent} />
      </button>

      {/* Beat counter + chord */}
      <div className="flex items-baseline gap-3 min-w-[8rem]">
        <span className="font-mono text-sm text-chrome-100">
          {hasContent ? String(currentBeat + 1).padStart(3, '0') : '—'}
          <span className="text-chrome-500"> / {hasContent ? String(beats.length).padStart(3, '0') : '—'}</span>
        </span>
        {matchedChord && (
      ...
      <div
        ...
        className={`flex-1 h-8 flex items-center group ${hasContent ? 'cursor-pointer' : ''}`}
      >
        <div className="relative w-full h-1 rounded-full bg-ink-700 overflow-visible">
          <div
            ...
            style={{ width: `${progress * 100}%`, transition: isPlaying ? 'none' : 'width 120ms ease' }}
          />
          {hasContent && (
            <div
      ...
      <div className="flex items-center gap-0.5 bg-ink-850 rounded-lg p-0.5 border border-ink-700/50">
        {[{ v: 0.5, l: '½×' }, { v: 0.75, l: '¾×' }, { v: 1, l: '1×' }].map(({ v, l }) => (
          <button
            key={v}
